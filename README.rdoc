...
8.7 CarrierWave_generowanie_miniatur_i_obsluga_plikow
    * Rozbudujemy nasza strone o mozliwosc dodawania obrazkow
     - musimy miec jeden z komponentów IMAGE MAGIC
     - pierwsze co musimy dodac to gem:
          ...
          gem 'mini_magick'  #gem wspomagajacy inter-reakcjie z imagemagiciem - aby generowac miniaturki obrazkow ktore sa zbyd duze
            ...
            CarrierWave - udostepnia nam odpowiedni uploader robimy to w ten sposob:
            skorzystamy z:
               https://rubygems.org/gems/carrierwave
               a tu z dokumentacji:
               http://www.rubydoc.info/gems/carrierwave/1.0.0
                  - nastepnie uzyjemy generatora

                   rails generate uploader picture

            Railsy stworzyly plik i umiescily go w :
             app/uploaders/picture_uploader.rb
             - wejdzmy do niego
             - mamy tutaj plik i wiele zakomentowanych linijek
         *** - chcemy ze zakazdym razem jak uzytkownik uplouduje jakis plik i my stworzymy sobie miniaturke
             - odkomuntujemy pola
                       version :thumb do
                           process :resize_to_fit => [50, 50]
                       end
         *** - a takze chcemy aby uzytkownik mogl wgrac tylko pewne typy plikow o konkretnych rozszerzeniach
             - odkomentujemy pola
                       def extension_white_list
                           %w(jpg jpeg gif png)
                       end
         *** - kiedy uzytkownik nie zaladuje zadnego obrazka,
               i wtedy wyswietli sie pusty obrazek i bedzie widoczny blad
               aby tego uniknac bedziemy wstawiac taka zaslepke.
                 - zwrocimy sciezke do obrazka jezeli w danym modelu nie bedzie
                   zaladowany zaden obrazek
                   damy sciezke ktora korzysta z serwisu plachod - ktory ma zadanie
                   generowanie zaslepek
                       def default_url
                   ...
                   *-* Czyli jezeli mamy obrazek to bedzie zwracana miniaturka docelowo
                       a jezeli nie to bedzie tworzona zaslepka z strony placehold.it

     * Teraz nasz PictuerUploader - musimy dodac do naszego modelu Post

                      mount_uploader :picture, PictureUploader
                        # korzystamy z dokumentacji z carrierwave
                        # nasz atrybut w ktorym beda przechowywane sciezki do plikow to picture
                        # a drugi argument to klasa PictureUploader

     * Nie obedzie sie takze od migracji do naszego modelu
         rails g migration add_picture_to_posts
           - wchodzimy do naszej migracji i dodajemy kolumne picture do tabeli posts
                db/migrate/20170109172625_add_picture_to_posts.rb
                       def change
                          add_column :posts, :picture, :string
                       end
           - uruchamiamy migracjie rake db:migrate
           - uruchamiamy nasz serwer
        - Chcemy teraz zeby w naszym formularzu pojawilo sie pole do dodawania zdjecia
               <div class="form-group">
                   <%= f.label :picture, "Obrazek" %>
                   <%= f.file_field :picture, class: "form-control" %>
               </div>

               lub jezeli uzywamy simple_form

               <%= f.input :picture, label: "Obrazek" %>

        - Dodatkowo musimy pamietac o tym zeby w naszym kontrolerze dodac nasz atrybut
          przechowujacy zdjecie - picture
                def post_params
                   params.require(:post).permit(:author, :body, :picture)
                end

        - Musialem odznaczyc kilka pol

           include CarrierWave::MiniMagick   - to nam zaladuje nasz dodany gem

           version :thumb do
             process :resize_to_fit => [50, 50]
           end

      * Wszystkie pliki-zdjecia loduja w public/uploads/post/picture/1/nazwa_zdjecia

      ** Teraz dodamy widok naszego obrazka w views/posts/index
              <% @posts.each do |post| %>
                <div class="row">
                  <div class="col-md-4">
              ...
              <% end %>
     ** To tyle jezeli chodzi o generoanie plikow i minjatur obrazkow

8.8 ActiveJob_delayed_job-przetwarzanie_asynchronicze
      ** Gdy wejdziemy na jakas strone to bedziemy pobierac adresy url
         - gdzie wpisujemy adres url
         np: http://google.pl
         - i klikamy dodaj wpis
         - wtedy zostanie pobrana strona
         - ale bedzie ona pobierana bardzo dlugo dlatego zrobimy to asynchronicznie
         - przeniesiemy czynnosc pobierania tej strony do innego procesu
         - nasz model uzywa callbecka
         - ktory czeka chwile (dzieki jenej z metod)
         - nastepnie pobiera dane strony
         - i znowu czeka aby pokazac jak wolno dziala


    * Storzylem sobie nowy model oraz controller
          rails g model Download url:string body:text
          rake db:migrate
          rails g controller downloads index new
               dodalem metody:
                          def index
          ...
          - dodalme routsy
              resources :downloads
          - dodalem nowe widoki index new
          index
               <h1>Lista wpisów</h1>

               <p class="lead">
                 Witaj w pobierarce wpisów!
                 <%= link_to "Dodaj własny wpis już teraz.", new_download_path %>
               </p>
               ...
               <table class="table table-striper">
                 ...
                 <% @downloads.each do |download| %>
                     <tr>
                       <td><%= download.url %></td>
                       <td><%= simple_format(truncate(download.body, length: 250)) %></td>
                     </tr>
                 <% end %>
                 </tbody>
               </table>
          ...
          new
              <h1>Dodaj wpis do pobrania</h1>

              <%= form_for @download do |f| %>
                  <div class="form-group">
                    <%= f.label :url, "Adres URL" %>
                    <%= f.text_field :url, class: "form-control" %>
                  </div>
                  <%= f.submit "Dodaj wpis", class: "btn btn-default" %>
              <% end %>
          rails s
          dodaj wlasny wpis juz teraz
          - i wyskakuje nam oczekiwany wstenpy efekt

        * ale jeszcze trzeba dodac metode do naszego modelu/Download
                after_create :fetch_body
                 private
                 def fetch_body
                   sleep 2        - tutaj spowanlniamy sobie pobieranie
                   fetch_body = Net::HTTP.get(URI.parse(self.url))
                   update_attribute(:body, fetch_body.force_encoding('UTF-8'))
                   sleep 2
                 end

        ** ABY NAM POBIERALO ASYNCHRONICZNIE UZYJEMY gema :
           ...
           - gem 'delayed_job_active_record' - jest w duzym skurcie sytemem do kolejkowania
             zadan/przechowywania zadan w aktwiw rekordzie

             nastepnie korzystamy z dokumentacji:
              rails generate delayed_job:active_record
              rake db:migrate

              * Dodatkowo musimy dodac taki wpis od kiedy wprowadzono activ rekorda
              config/application.rb
              config.active_job.queue_adapter = :delayed_job
                -> I TO  SPR. ZE RAILSY BEDA KORZYSTALY Z DLEAYTEDJOBA do kolejkowania zadan

        ** Kazde zadanie przetwarzane w railsach asynchronicznie nazywa sie JOB
           i railsy oczywiscie posiadaja generator
               rails g job post_fetch    ->czyli pobranie posta
               rake db:migrate

               - i widzimy ze do katalogu app/jobs/ zostal dodany nasz post_fetch
                  queue_as :default
                    def perform(*args)
                      # Do something later
                    end
               - widzimy ze mamy tutaj standardowa metode perform
               - do tej metody dopisujemy to co ma byc poberane asynchronicznie

        ** Czyli juz mamy metode w Job to usowamy ja z models/download.rb
        *  -i w momecie kiedy post zostanie stworzony chcemy zakolekowac nasze pobieranie
             zeby zostala pobrana puzniej
                ...
                class Download < ActiveRecord::Base
                  ...
                  private
                  def fetch_body
                     PostFetchJob.perform_later(self)   - i to spowoduje ze ona bedzie wyslana do kolejki i zostanie pobrana kiedy skacza sie inne zadania zakolejkowane  (self - oznacza, aktualny argument czyli nasz Post)_
                  end
                end
        ** A dajemy w naszym app/jobs/post_fetch_job.rb
        ...
        * rails s
        * otwieramy druga karte w terminalu i w niej bedzeimy widziec zakolejkowanie procesy
           rake jobs:work
             - no i widzimy ze tu jest worker, ktory czeka na zadania które do niego wpadna
        ** I teraz kiedy klikniemy dodamy nowy wpis, widzimy ze wpis dodal sie natychmiast lecz nie widoczna jest jescze reszta
           - nie zostala jeszcze przetworzona
           - czekamy chwile i po odswiezeniu strony widzimy ze wpis sie pojawil
